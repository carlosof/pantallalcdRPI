#!/usr/bin/env python
"""Añadir la libreria de python con: python -m easy_install --user https://github.com/pl31/python-liquidcrystal_i2c/archive/master.zip """
import socket
import os.path
import sys
import struct
...
if os.path.isfile(PATH1) and os.access(PATH1, os.R_OK):
    efg = open(PATH1).read()
    pstr = 'MAC'+open('/sys/class/net/eth0/address').read()
    pstr = pstr.strip('\n')
    lcd.printline(2,pstr)
    if efg[0]=="1":
        pstr='IP:'+str(getip('eth0'))
